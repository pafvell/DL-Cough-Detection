        	...
        	#with slim.arg_scope(batchnorm_arg_scope()): 
				      ...
				      #random block
                                      with tf.variable_scope('middle'):
                                                ...
                                                net = slim.max_pool2d(net, [2, 2], stride=[2, 2], scope='poolM')
...
def build_model(x, 
	...
	here we do boosting without additive training
        ...
        #model	
        logits = classify(x, num_estimator=num_estimator, num_classes=num_classes, is_training=is_training, reuse=reuse, scope='H0')

        for i in range(1,num_estimator):
                #logits = tf.stop_gradient(logits)
                predictions = classify(x, num_estimator=num_estimator, num_classes=num_classes, is_training=is_training, reuse=reuse, scope='H%d'%(i+1))
                #zeta = 2. / (i+2) 
                logits = logits + predictions #(1-zeta) * logits + zeta * predictions
...
#Parameters
TRAINABLE_SCOPES = ['top'] #top is trainable
