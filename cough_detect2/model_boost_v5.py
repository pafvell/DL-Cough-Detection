...
def build_model(x, 
	...
	here we do boosting without additive training
        ...
        #model	
        predictions = classify(x, num_estimator=num_estimator, num_classes=num_classes, is_training=is_training, reuse=reuse, scope='H0')
        logits = predictions
        ...
        for i in range(1,num_estimator):
                ...
                zeta = 2. / (i+2) 
                logits = (1-zeta) * logits + zeta * predictions  #logits + predictions #
                loss += loss_fkt(logits, y)
