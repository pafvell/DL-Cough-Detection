		                	...
		                	is_training=is_training):
                             with tf.variable_scope(scope, 'model_v1', [inputs], reuse=reuse) as scope:
                                      ...
                                      with tf.variable_scope('bottom'):
                                                ...
                                                net = slim.repeat(net, bottom_size, slim.conv2d, num_filter, [3, 9], scope='convB2')
                                                net = slim.max_pool2d(net, [1, 2], stride=[1, 2], scope='poolB1')

				      #random block
                                      with tf.variable_scope('middle'):
                                                net = slim.repeat(net, middle_size, slim.conv2d, num_filter, [3, 5], scope='convM1') #, reuse=i>0
                                                net = slim.max_pool2d(net, [1, 2], stride=[1, 2], scope='poolM')

                                                net = slim.repeat(net, middle_size, slim.conv2d, num_filter, [3, 3], scope='convM2') #, reuse=i>0
                                                net = slim.max_pool2d(net, [1, 2], stride=[1, 2], scope='poolM')

                                                net = slim.repeat(net, middle_size, slim.conv2d, num_filter, [3, 3], scope='convM3') #, reuse=i>0
                                                net = slim.max_pool2d(net, [1, 2], stride=[1, 2], scope='poolM')
