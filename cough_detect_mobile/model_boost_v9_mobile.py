                             ...
                             with tf.variable_scope(scope, 'weak_learner', [inputs], reuse=reuse) as scope:
                                      ...
                                      with tf.variable_scope('middle'):
                                                ...
                                                for i in range(route):
                                                      ...
                                                      net += slim.separable_conv2d(net, num_filter, [3, 3], depth_multiplier=1, activation_fn=None, scope='conv3x3_%d'%(i+2)) 
                                                      net = tf.nn.relu(net)
	...
	here we do boosting without additive training
        ...
        with tf.variable_scope('model_v1'):
             ...
             for i in range(num_estimator):
                logits += classify(x, num_classes=num_classes, num_filter=num_filter, is_training=is_training, reuse=reuse, route=3-int(i%4), scope='H%d'%i)
