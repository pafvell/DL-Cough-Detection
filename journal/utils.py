...
def extract_Signal_Of_Importance(signal, window, sample_rate):
    ...
    :param signal: input
    :param window -> size of a window sample_rate
    :param sample_rate:
    """
...
def preprocess_array(sound_file_array, sample_rate,
                     bands,
                     hop_length,
                     window,
                     nfft
                     ):
    time_signal = standardize(sound_file_array)
    mfcc = librosa.feature.melspectrogram(y=time_signal, sr=sample_rate, n_mels=bands, power=1,
...
def computeLocalHuMoments(stardardized_time_signal, sample_rate, hop_length=512):
    ...
    w = 5
    mfcc = librosa.feature.melspectrogram(y=stardardized_time_signal, sr=sample_rate, n_mels=75, power=1,
                                          hop_length=hop_length, n_fft=4096)
